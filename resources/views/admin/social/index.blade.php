...
<div class="bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl shadow-lg p-6 mb-8 text-white">
    <div class="flex items-center justify-between">
        ...
        <button id="add-social-link-btn" class="bg-white text-blue-600 hover:bg-blue-50 px-4 py-2 rounded-lg font-medium transition-colors flex items-center">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-100 dark:border-gray-700">
        ...
        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $socialLinks->where('is_active', true)->count() }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-100 dark:border-gray-700">
        ...
        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $socialLinks->where('is_active', false)->count() }}</p>
    </div>
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
            <button id="bulk-edit-btn" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors" title="Bulk Edit">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($socialLinks as $link)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ $link->url }}" target="_blank" class="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 truncate max-w-xs block" title="{{ $link->url }}">
                                {{ Str::limit($link->url, 30) }}
                                <i class="fas fa-external-link-alt ml-1"></i>
                            </a>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            <code class="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-xs">{{ $link->icon }}</code>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ $link->order }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ $link->url }}" target="_blank" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300" title="Visit Link">
                                    <i class="fas fa-external-link-alt"></i>
                                </a>
                                <button class="edit-link-btn text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300" title="Edit" data-link='{{ json_encode($link) }}'>
                                    <i class="fas fa-edit"></i>
                                </button>
                                <form action="{{ route('admin.social.destroy', $link) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this social link?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No social links yet</h3>
                                <p class="text-gray-500 dark:text-gray-400 mb-4">Get started by adding your first social media link</p>
                                <button id="add-first-link-btn" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full">
            ...
            <form id="social-link-form" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="url" id="url-input" name="url" class="form-input" placeholder="https://example.com" required>
                </div>

                <div class="mb-4">
                    <label class="form-label">Icon Class</label>
                    <input type="text" id="icon-input" name="icon" class="form-input" placeholder="ri-twitter-fill" required>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Use RemixIcon classes (e.g., ri-twitter-fill)</p>
                </div>

                <div class="mb-4">
                    <label class="form-label">Order</label>
                    <input type="number" id="order-input" name="order" class="form-input" min="1" value="1" required>
                </div>

                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" id="active-checkbox" name="is_active" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function openModal(link = null) {
        ...
        if (link) {
            ...
            document.getElementById('active-checkbox').checked = link.is_active;
            form.action = `/admin/social/${link.id}`;
            form.method = 'POST';
