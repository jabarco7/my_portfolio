...
@section('content')
    ...
    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl shadow-lg p-6 mb-8 text-white">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold mb-2">Create New Certificate</h1>
                <p class="text-blue-100 opacity-90">Add a new certificate to your portfolio</p>
            </div>
        ...
        <form action="{{ route('admin.certificates.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="4"
                    ...
                    placeholder="Brief description of the certificate">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
