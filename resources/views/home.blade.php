    ...
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div class="order-1 md:order-2 flex justify-center">
                <img src="{{ asset('assets/ph1.jpeg') }}" alt="Profile"
...
<section id="portfolio" class="py-20 bg-white dark:bg-gray-900">
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @if($projects->count() > 0)
                @foreach($projects->take(6) as $project)
                    <a href="{{ route('projects.show', $project->slug) }}" class="group relative overflow-hidden rounded-lg shadow-lg block no-underline">
                        @if($project->featured_image)
                            <img src="{{ asset('storage/' . $project->featured_image) }}"
                                 alt="{{ $project->title }}" class="w-full h-64 object-cover transition-transform duration-300 group-hover:scale-110">
                        @else
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <div class="absolute bottom-0 left-0 right-0 p-6 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                                ...
                                <p class="text-sm mb-4 leading-relaxed">{{ $project->excerpt ?? Str::limit($project->description, 100) }}</p>
                                <div class="flex space-x-4">
                                    <span class="text-white hover:text-primary-300 transition-colors font-medium">
                                        <i class="fas fa-eye mr-1"></i> View Details
                                    </span>
                                    @if($project->project_url)
                                        <a href="{{ $project->project_url }}" target="_blank" class="text-white hover:text-primary-300 transition-colors font-medium no-underline" onclick="event.stopPropagation()">
        ...
        <div class="text-center mt-12">
            <a href="{{ route('portfolio') }}" class="px-8 py-3 rounded-md font-medium transition-all duration-300 transform hover:scale-105 hover:shadow-lg projects-btn text-base font-semibold tracking-wide no-underline" style="background-color: var(--color-primary) !important; color: var(--color-base-content) !important; border-color: var(--color-primary) !important;">
                إظهار جميع المشاريع
            </a>
        </div>
        @endif
