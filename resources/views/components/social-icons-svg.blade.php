...
<div class="flex justify-center gap-4 {{ $class ?? '' }}">
    @foreach (config('portfolio.social_links') as $social)
        <a href="{{ $social['url'] }}" 
           class="group flex items-center justify-center w-12 h-12 rounded-full bg-base-200 border border-base-300 hover:bg-primary hover:border-primary transition-all duration-300 hover:scale-110" 
           target="_blank" 
           ...
           title="{{ $social['label'] }}">
            ...
            @elseif($social['label'] === 'Email')
                <svg class="w-6 h-6 {{ $social['color'] ?? 'text-base-content' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
